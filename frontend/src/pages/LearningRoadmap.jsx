...
const LearningRoadmap = () => {
  ...
  return (
    <div className="container max-w-7xl mx-auto px-4 py-8 space-y-8 font-['Outfit']">
      <section className="relative overflow-hidden rounded-3xl border border-teal-300/30 bg-[linear-gradient(120deg,rgba(20,184,166,0.10),rgba(251,191,36,0.08),rgba(15,23,42,0.04))] p-8 sm:p-10">
        ...
        <Card className="xl:col-span-2 border-border/60 shadow-sm overflow-hidden">
          ...
          <CardContent>
            <div className="w-full overflow-x-auto">
              <div className="min-w-[900px] h-[620px] relative rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50">
                <svg className="absolute inset-0 h-full w-full" viewBox="0 0 900 620" preserveAspectRatio="none" aria-hidden>
                  {ROADMAP_NODES.flatMap((node) =>
                    node.deps.map((depId) => {
                      const depNode = ROADMAP_NODES.find((n) => n.id === depId);
                      if (!depNode) return null;

                      const depStatus = nodeState[depId]?.status;
                      const stroke = depStatus === 'mastered' ? '#10b981' : depStatus === 'in-progress' ? '#f59e0b' : '#94a3b8';

                      return (
                        <path
                          key={`${depId}-${node.id}`}
                          d={`M ${depNode.x + 90} ${depNode.y + 30} C ${depNode.x + 190} ${depNode.y + 30}, ${node.x - 90} ${node.y + 30}, ${node.x} ${node.y + 30}`}
                          fill="none"
                          stroke={stroke}
                          strokeOpacity="0.55"
                          strokeWidth="3"
                          strokeDasharray={depStatus === 'locked' ? '8 7' : '0'}
                        />
                      );
                    })
                  )}
                ...
                {ROADMAP_NODES.map((node) => {
                  ...
                  return (
                    <button
                      ...
                      className={`absolute w-[180px] rounded-2xl border p-3 text-left transition-all duration-300 ${
                        selected ? 'ring-2 ring-teal-500 border-teal-400 shadow-xl' : 'border-slate-200 shadow-sm hover:shadow-md'
                      } ${state.status === 'locked' ? 'bg-slate-100/95' : 'bg-white/95'}`}
                      style={{ left: `${node.x}px`, top: `${node.y}px` }}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-black text-slate-900 leading-tight text-sm">{node.label}</p>
